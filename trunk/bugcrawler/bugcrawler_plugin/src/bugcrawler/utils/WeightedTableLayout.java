...
public class WeightedTableLayout extends TableLayout {
    ...
    /**
         * This constructor calls {@link #WeightedTableLayout(int[], int[]) }
         * with <code>fixedWidths == null</code>
         */
    ...
    /**
     * @param weights
     *                Each weight defines the proportional width of the
     *                column (by zero-based column index). If the column is
     *                not defined (i.e. the <code>weights</code> array is
     *                too short), a weight of 0 is assumed. If one column
     *                has a fixed width, its weight must be -1 (which is an
     *                illegal argument for a variable-width column).
     * @param fixedWidths
     *                Each column that is represented with a fixedWidth >= 0
     *                is excluded from variable-width-calculation and always
     *                has the same width (useful for image-only columns).
     */
